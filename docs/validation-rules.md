...
The defense is incomplete without code signing: unsigned installers can be modified in transit to ship a permissive `config.json` or a tampered validator. Code signing and notarization track in [M5 — Signing & Distribution](https://github.com/manomatika/ahimsa/milestone/10) and are required before any external distribution.
